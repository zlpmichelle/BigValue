...
public class CoreWorkload extends Workload
{
	...
	/**
	 * plug random generator for rowkey and indexfield
	 */
	public static final String RAND_PLUGIN_PROPERTY = "plug";
	
	public static final String RAND_PLUGIN_DEFAULT = "false";
	
	public static boolean randplug;
	
	/**
	 * hint if build index or not
	 */
	public static final String BUILD_INDEX_PROPERTY = "buildindex";
	
	public static final String BUILD_INDEX_DEFAULT = "false";
	 ...
	 */
	public static final String IDX_TABLENAME_PROPERTY = "indextable";
	
	public static final String IDX_TABLENAME_DEFAULT = "usertable_index";
	...
	public static final String IDX_FIELDNAME_PROPERTY = "indexfield";
	
	public static final String IDX_FIELDNAME_DEFAULT="field0";
  ...
  public static final String HOTSPOT_OPN_FRACTION_DEFAULT = "0.8";
	...
	public void init(Properties p) throws WorkloadException
	{
		...
		p.setProperty(TABLENAME_PROPERTY, table);
		
		randplug = Boolean.parseBoolean(p.getProperty(RAND_PLUGIN_PROPERTY, RAND_PLUGIN_DEFAULT));
		
		buildindex = Boolean.parseBoolean(p.getProperty(BUILD_INDEX_PROPERTY, BUILD_INDEX_DEFAULT));
		
		indextable = p.getProperty(IDX_TABLENAME_PROPERTY, IDX_TABLENAME_DEFAULT);
		if(buildindex){
			p.setProperty(IDX_TABLENAME_PROPERTY, indextable);
		}
		
		indexfield = p.getProperty(IDX_FIELDNAME_PROPERTY, IDX_FIELDNAME_DEFAULT);
	...
	public boolean doInsert(DB db, Object threadstate)
	{
		...
		if(buildindex){
			if(!doIndexInsert(db, dbkey, values, threadstate)){
				return false;
			}
		}
	...
	/**
	 * If buildindex is true, then workload needs to build the index while inserting records
	 */
	public static byte[] DUMMY_BYTE_ARRAY = new byte[]{(byte)127};
	public boolean doIndexInsert(DB db, String dbkey, HashMap<String, ByteIterator> values, Object threadstate){
		ByteIterator iter = values.get(indexfield);
		byte[] value = iter.toArray();
		values.put(indexfield, new ByteArrayByteIterator(value)); // write ByteArrayByteIterator back
		
		String indexkey = new String(value) + "|" + dbkey;
		if(db.insert(indextable, indexkey, buildIndexValues()) == 0){
			return true;
		}else{
			return false;
		}
	...
	public HashMap<String, ByteIterator> buildIndexValues(){
		HashMap<String, ByteIterator> indexValues = new HashMap<String, ByteIterator>();
		ByteArrayByteIterator byteIterator = new ByteArrayByteIterator(DUMMY_BYTE_ARRAY);
		
		indexValues.put("field0", byteIterator);
